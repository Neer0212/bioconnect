...
export default function EventsPage() {
  ...
  const now = new Date();
  const typeColors = { conference: { bg: "rgba(249,115,22,0.1)", color: "#F97316" }, webinar: { bg: "rgba(6,182,212,0.1)", color: "#06B6D4" }, workshop: { bg: "rgba(168,85,247,0.1)", color: "#A855F7" }, seminar: { bg: "rgba(59,130,246,0.1)", color: "#3B82F6" }, hackathon: { bg: "rgba(236,72,153,0.1)", color: "#EC4899" }, other: { bg: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.5)" } };
  ...
  return (
    <main style={S.page}><style>{CSS}</style>
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          {[{ label: "Dashboard", href: "/dashboard" }, { label: "Learning", href: "/learning" }, { label: "Research", href: "/research" }, { label: "Events", href: "/eventss" }].map((item) => (
            <a key={item.label} href={item.href} style={{ fontSize: "14px", color: item.label === "Events" ? "#F97316" : "rgba(255,255,255,0.4)", padding: "6px 14px", borderRadius: "8px", fontWeight: item.label === "Events" ? 600 : 400, background: item.label === "Events" ? "rgba(249,115,22,0.1)" : "transparent", textDecoration: "none" }}>{item.label}</a>
          ))}
        </div>
      ...
      <section style={{ textAlign: "center", padding: "48px 20px 24px" }}>
        <h1 style={S.h1}>Events & Conferences</h1>
        <p style={S.sub}>Stay updated on biotech events, webinars, and workshops</p>
      </section>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", gap: "6px" }}>
            {["all", "upcoming", "past"].map((f) => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: 500, border: "1px solid", cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif", background: filter === f ? "linear-gradient(135deg,#F97316,#EA580C)" : "rgba(255,255,255,0.03)", color: filter === f ? "#fff" : "rgba(255,255,255,0.4)", borderColor: filter === f ? "#F97316" : "rgba(255,255,255,0.08)" }}>{f.charAt(0).toUpperCase() + f.slice(1)}</button>
            ))}
          </div>
          {isEducator && (<button onClick={() => setShowAdd(!showAdd)} style={{ background: "linear-gradient(135deg,#F97316,#EA580C)", color: "#fff", border: "none", padding: "10px 20px", borderRadius: "10px", fontSize: "14px", fontWeight: 500, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>{showAdd ? "Cancel" : "+ Add Event"}</button>)}
        </div>
      </section>
      ...
      {showAdd && (
        <section style={{ padding: "0 max(24px, calc((100vw - 800px)/2))", marginBottom: "24px" }}>
          <div style={S.card}>
            <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#fff", marginBottom: "20px" }}>Add New Event</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              ...
              <div style={{ gridColumn: "1 / -1" }}><button onClick={handleAdd} disabled={saving || !form.title || !form.event_date} style={{ background: "linear-gradient(135deg,#F97316,#EA580C)", color: "#fff", border: "none", padding: "12px 24px", borderRadius: "10px", fontSize: "14px", fontWeight: 500, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif", opacity: saving || !form.title || !form.event_date ? 0.6 : 1 }}>{saving ? "Adding..." : "Add Event"}</button></div>
            </div>
          </div>
        </section>
      )}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px", flexWrap: "wrap" }}>
                        <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#fff" }}>{ev.title}</h3>
                        ...
                        {isPast && <span style={{ fontSize: "11px", fontWeight: 500, color: "rgba(255,255,255,0.3)", background: "rgba(255,255,255,0.05)", padding: "3px 10px", borderRadius: "100px" }}>Past</span>}
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", marginBottom: "10px", fontSize: "13px", color: "rgba(255,255,255,0.4)" }}>
                      ...
                      {ev.description && <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", lineHeight: "1.6", marginBottom: "8px" }}>{ev.description}</p>}
                      <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.25)" }}>Posted by {ev.profiles?.full_name || "Unknown"}</span>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", flexShrink: 0 }}>
                      ...
                      {isEducator && ev.created_by === profile?.id && (<button onClick={() => handleDelete(ev.id)} style={{ fontSize: "13px", color: "#EF4444", fontWeight: 500, padding: "8px 16px", background: "rgba(220,38,38,0.1)", borderRadius: "8px", border: "none", cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>Delete</button>)}
                    </div>
                  </div>
                </div>
              );

  ...
  return (
    <AppShell active="/research">
      {({ profile, supabase }) => {
        ...
        async function loadPapers() { const { data } = await supabase.from("research_papers").select("*, profiles(full_name)").order("created_at", { ascending: false }); setPapers(data || []); setLoaded(true); }
          ...
          <>
            ...
            {filtered.length === 0 ? <div style={{ textAlign: "center", padding: "60px 20px", color: "#9ca3af" }}><p>{search ? "No papers match" : "No papers yet"}</p></div> : (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px" }}>
                {filtered.map(paper => (
                  <div key={paper.id} style={{ background: "#fff", border: "1px solid #e7e5e4", borderRadius: "14px", padding: "24px", display: "flex", flexDirection: "column", transition: "all 0.3s" }}>

  ...
  return (
    <main style={S.page}><style>{CSS}</style>
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          {[{ label: "Dashboard", href: "/dashboard" }, { label: "Learning", href: "/learning" }, { label: "Research", href: "/research" }, { label: "Events", href: "/eventss" }].map((item) => (
            <a key={item.label} href={item.href} style={{ fontSize: "14px", color: item.label === "Research" ? "#F97316" : "rgba(255,255,255,0.4)", padding: "6px 14px", borderRadius: "8px", fontWeight: item.label === "Research" ? 600 : 400, background: item.label === "Research" ? "rgba(249,115,22,0.1)" : "transparent", textDecoration: "none" }}>{item.label}</a>
          ))}
        </div>
      ...
      <section style={{ textAlign: "center", padding: "48px 20px 24px" }}>
        <h1 style={S.h1}>Research Papers</h1>
        <p style={S.sub}>Browse scientific publications from the biotech community</p>
      </section>

      <section style={{ padding: "0 max(24px, calc((100vw - 800px)/2))", marginBottom: "24px" }}>
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          <input type="text" placeholder="Search by title, author, topic, or journal..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ flex: 1, padding: "12px 16px", border: "1.5px solid rgba(255,255,255,0.08)", borderRadius: "10px", fontSize: "14px", fontFamily: "'Plus Jakarta Sans',sans-serif", outline: "none", background: "rgba(255,255,255,0.04)", color: "#fff" }}/>
          {isEducator && (<button onClick={() => setShowAdd(!showAdd)} style={{ background: "linear-gradient(135deg,#F97316,#EA580C)", color: "#fff", border: "none", padding: "12px 20px", borderRadius: "10px", fontSize: "14px", fontWeight: 500, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif", whiteSpace: "nowrap" }}>{showAdd ? "Cancel" : "+ Add Paper"}</button>)}
        </div>
      </section>
      ...
      {showAdd && (
        <section style={{ padding: "0 max(24px, calc((100vw - 800px)/2))", marginBottom: "24px" }}>
          <div style={S.card}>
            <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#fff", marginBottom: "20px" }}>Add Research Paper</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              ...
              <div style={{ gridColumn: "1 / -1" }}><button onClick={handleAdd} disabled={saving || !form.title} style={{ background: "linear-gradient(135deg,#F97316,#EA580C)", color: "#fff", border: "none", padding: "12px 24px", borderRadius: "10px", fontSize: "14px", fontWeight: 500, cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif", opacity: saving || !form.title ? 0.6 : 1 }}>{saving ? "Adding..." : "Add Paper"}</button></div>
            </div>
          </div>
        </section>
      )}
            ...
            {filtered.map((paper) => (
              <div key={paper.id} style={S.card}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px" }}>
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#fff", marginBottom: "6px" }}>{paper.title}</h3>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginBottom: "10px" }}>
                      {paper.journal && <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", display: "flex", alignItems: "center", gap: "4px" }}>📖 {paper.journal}</span>}
                      {paper.topic && <span style={{ fontSize: "12px", background: "rgba(249,115,22,0.1)", color: "#F97316", padding: "2px 10px", borderRadius: "100px", fontWeight: 500 }}>{paper.topic}</span>}
                      {paper.published_date && <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>📅 {new Date(paper.published_date).toLocaleDateString("en-IN", { year: "numeric", month: "short" })}</span>}
                    </div>
                    {paper.abstract && <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", lineHeight: "1.6", marginBottom: "8px" }}>{paper.abstract}</p>}
                    <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.25)" }}>Added by {paper.profiles?.full_name || "Unknown"}</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px", flexShrink: 0 }}>
                    {paper.paper_url && <a href={paper.paper_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: "13px", color: "#F97316", fontWeight: 500, padding: "8px 16px", background: "rgba(249,115,22,0.1)", borderRadius: "8px", textDecoration: "none", textAlign: "center" }}>View Paper</a>}
                    {(profile?.is_admin || (isEducator && paper.uploaded_by === profile?.id)) && (<button onClick={() => handleDelete(paper.id)} style={{ fontSize: "13px", color: "#EF4444", fontWeight: 500, padding: "8px 16px", background: "rgba(220,38,38,0.1)", borderRadius: "8px", border: "none", cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>Delete</button>)}
                  </div>
                </div>
              </div>
            ))}
...
const S = {
  ...
  card: { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "24px 28px" },
  label: { display: "block", fontSize: "13px", fontWeight: 500, color: "rgba(255,255,255,0.5)", marginBottom: "6px" },
  input: { width: "100%", padding: "10px 14px", border: "1.5px solid rgba(255,255,255,0.08)", borderRadius: "10px", fontSize: "14px", fontFamily: "'Plus Jakarta Sans',sans-serif", outline: "none", background: "rgba(255,255,255,0.04)", color: "#fff" },
};
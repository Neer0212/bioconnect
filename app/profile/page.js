  ...
  return (
    <main style={S.page}><style>{CSS}</style>
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          {[{ label: "Dashboard", href: "/dashboard" }, { label: "Learning", href: "/learning" }, { label: "Research", href: "/research" }, { label: "Events", href: "/eventss" }].map((item) => (
            <a key={item.label} href={item.href} style={{ fontSize: "14px", color: "rgba(255,255,255,0.45)", padding: "6px 14px", borderRadius: "8px", fontWeight: 400, background: "transparent", textDecoration: "none" }}>{item.label}</a>
          ))}
        </div>
      ...
      <section style={{ padding: "48px max(24px, calc((100vw - 600px)/2)) 80px" }}>
        ...
        {message && <div style={{ padding: "10px 14px", borderRadius: "10px", fontSize: "13px", marginBottom: "16px", textAlign: "center", background: message.includes("Failed") ? "#FEF2F2" : "#F0FDF4", color: message.includes("Failed") ? "#DC2626" : "#16A34A", border: `1px solid ${message.includes("Failed") ? "#FECACA" : "#BBF7D0"}` }}>{message}</div>}

        <div style={S.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
            <h2 style={{ fontSize: "18px", fontWeight: 600, color: "#1c1917" }}>Profile Details</h2>
            {!editing ? <button onClick={() => setEditing(true)} style={S.editBtn}>Edit Profile</button> : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            {[
              ...
              <div key={f.l}>
                ...
                {editing && f.editable ? (f.textarea ? <textarea value={form[f.k]} onChange={(e) => setForm({ ...form, [f.k]: e.target.value })} placeholder={f.ph} rows={3} style={{ ...S.input, resize: "vertical" }}/> : <input value={form[f.k]} onChange={(e) => setForm({ ...form, [f.k]: e.target.value })} placeholder={f.ph} style={S.input}/>) : <p style={S.value}>{f.v || "—"}</p>}
...
const S = {
  ...
  card: { background: "#fff", border: "1px solid #e7e5e4", borderRadius: "16px", padding: "32px 28px", boxShadow: "0 4px 24px rgba(0,0,0,0.04)" },
  label: { display: "block", fontSize: "13px", fontWeight: 500, color: "#9ca3af", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.5px" },
  value: { fontSize: "15px", color: "#44403c", fontWeight: 400 },
  input: { width: "100%", padding: "12px 14px", border: "1.5px solid #e7e5e4", borderRadius: "10px", fontSize: "14px", fontFamily: "'Plus Jakarta Sans',sans-serif", outline: "none", background: "#fff", color: "#1c1917" },
  editBtn: { background: "rgba(249,115,22,0.08)", border: "none", cursor: "pointer", fontSize: "13px", color: "#F97316", padding: "8px 18px", borderRadius: "8px", fontWeight: 500, fontFamily: "'Plus Jakarta Sans',sans-serif" },
  ...
  cancelBtn: { background: "#f5f5f4", border: "1px solid #e7e5e4", cursor: "pointer", fontSize: "13px", color: "#6b7280", padding: "8px 18px", borderRadius: "8px", fontWeight: 500, fontFamily: "'Plus Jakarta Sans',sans-serif" },
};
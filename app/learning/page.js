...
export default function LearningPage() {
  ...
  useEffect(() => { async function load() { const { data: { user } } = await supabase.auth.getUser(); if (!user) { router.push("/login"); return; } const { data } = await supabase.from("profiles").select("*").eq("id", user.id).single(); setProfile(data); loadAllNotes(); } load(); }, []);

  async function loadAllNotes() { const result = {}; for (const sub of SUBJECTS) { const { data } = await supabase.storage.from("course-materials").list(sub.id); result[sub.id] = data || []; } setNotes(result); }

  async function handleUpload(subjectId, file) { if (!file || !file.name.endsWith(".pdf")) { alert("Please upload a PDF file"); return; } setUploading(true); const fileName = `${profile.id.slice(0, 8)}_${Date.now()}_${file.name}`; const { error } = await supabase.storage.from("course-materials").upload(`${subjectId}/${fileName}`, file); if (error) { alert("Upload failed: " + error.message); } else { await loadAllNotes(); } setUploading(false); }
  ...
  return (
    <main style={S.page}><style>{CSS}</style>
      <nav style={S.nav}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          {[{ label: "Dashboard", href: "/dashboard" }, { label: "Learning", href: "/learning" }, { label: "Research", href: "/research" }, { label: "Events", href: "/eventss" }].map((item) => (
            <a key={item.label} href={item.href} style={{ fontSize: "14px", color: item.label === "Learning" ? "#F97316" : "rgba(255,255,255,0.45)", padding: "6px 14px", borderRadius: "8px", fontWeight: item.label === "Learning" ? 600 : 400, background: item.label === "Learning" ? "rgba(249,115,22,0.1)" : "transparent", textDecoration: "none" }}>{item.label}</a>
          ))}
        </div>
      ...
      <section style={{ textAlign: "center", padding: "48px 20px 24px" }}>
        ...
        <p style={{ fontSize: "16px", color: "#9ca3af" }}>Access study materials and notes organized by subject</p>
      </section>
      ...
      <section style={{ padding: "0 max(24px, calc((100vw - 800px)/2)) 60px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {SUBJECTS.map((sub) => {
            ...
            return (
              <div key={sub.id} style={{ background: "#fff", border: "1px solid #e7e5e4", borderRadius: "16px", overflow: "hidden" }}>
                <button onClick={() => setOpenSubject(isOpen ? null : sub.id)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", background: "none", border: "none", cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                    ...
                    <div style={{ textAlign: "left" }}><h3 style={{ fontSize: "16px", fontWeight: 600, color: "#1c1917", marginBottom: "2px" }}>{sub.name}</h3><span style={{ fontSize: "13px", color: "#9ca3af" }}>{pdfFiles.length} {pdfFiles.length === 1 ? "note" : "notes"}</span></div>
                  </div>
                  <span style={{ fontSize: "20px", color: "#d6d3d1", transition: "transform 0.2s", transform: isOpen ? "rotate(180deg)" : "rotate(0)" }}>▾</span>
                </button>
                {isOpen && (
                  <div style={{ padding: "0 24px 24px", borderTop: "1px solid #e7e5e4" }}>
                    {pdfFiles.length === 0 ? <p style={{ fontSize: "14px", color: "#9ca3af", padding: "20px 0 12px" }}>No notes uploaded yet for this subject.</p> : (
                      <div style={{ display: "flex", flexDirection: "column", gap: "8px", padding: "16px 0 12px" }}>
                        {pdfFiles.map((file) => (
                          <div key={file.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "#fafaf9", borderRadius: "10px", border: "1px solid #f5f5f4" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}><span style={{ fontSize: "18px" }}>📄</span><span style={{ fontSize: "14px", color: "#1c1917", fontWeight: 500 }}>{file.name.replace(/^[^_]+_\d+_/, "")}</span></div>
                            <div style={{ display: "flex", gap: "8px" }}>
                              <a href={getPublicUrl(sub.id, file.name)} target="_blank" rel="noopener noreferrer" style={{ fontSize: "13px", color: "#F97316", fontWeight: 500, padding: "6px 14px", borderRadius: "8px", background: "rgba(249,115,22,0.08)", textDecoration: "none" }}>View PDF</a>
                              {isEducator && <button onClick={() => handleDelete(sub.id, file.name)} style={{ fontSize: "13px", color: "#EF4444", fontWeight: 500, padding: "6px 14px", borderRadius: "8px", background: "#FEF2F2", border: "none", cursor: "pointer", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>Delete</button>}
                            </div>
                          </div>
                        ))}
                      </div>

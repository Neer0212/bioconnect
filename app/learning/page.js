...
export default function LearningPage() {
  ...
  async function handleUpload(sid, file) { if (!file || !file.name.endsWith(".pdf")) { alert("Please upload a PDF"); return; } setUploading(true); const fn = `${profile.id.slice(0,8)}_${Date.now()}_${file.name}`; const { error } = await supabase.storage.from("course-materials").upload(`${sid}/${fn}`, file); if (error) alert("Upload failed: " + error.message); else await loadAllNotes(); setUploading(false); }
  function getUrl(sid, fn) { return supabase.storage.from("course-materials").getPublicUrl(`${sid}/${fn}`).data.publicUrl; }
  async function handleDelete(sid, fn) { if (!confirm("Delete?")) return; await supabase.storage.from("course-materials").remove([`${sid}/${fn}`]); await loadAllNotes(); }
